...
- **List everything for a party — `metadata_filters` is the ENUMERATION surface too.** To LIST every document for a party rather than rank a topic, filter on the party field with a high `limit` instead of searching the party name and reading the ranking. `customer_name` lists the **sales-side** documents; an NDA, an insurance certificate or a payment record files the SAME party under a **different role field** (`counterparty_name`, `parties`, `certificate_holder`), and **filters are AND-only — there is no OR across fields**, so one predicate cannot span two of them. So pair the filtered call with **one unfiltered search for the party name**: every extracted fact value is projected into the searchable text, so the unfiltered pass surfaces the documents filed under the other role fields — but that pass is a relevance-ranked cross-check, capped and paginated, not a guarantee of completeness; paginate it, or add per-field filters, if completeness matters. Discover the workspace's real field names with `metadata action=fields-list` and read a stored value off a row's `facts` before you filter on it — `=` is exact with no `contains`, so a near-miss value returns a confident zero.
