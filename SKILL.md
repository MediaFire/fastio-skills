...
- **To tell similarly-named documents apart, pass `details=true`.** A bare hit carries a snippet and its rank position and NOTHING that identifies the document. Under `details`, `output` decides how much metadata comes back: `terse` gives field NAMES with no values, **`standard` (the default here) keeps values, abbreviated** — that is what distinguishes a `document_title` ending "(DRAFT)" from the executed agreement — and `full` adds provenance at roughly 5 KB per hit. Under `details=true`, a hit also carries a compact `facts` field with up to four identifying extracted values (`document_title`, `customer_name`, `company_name`, `effective_date`) when present.
- Under `details=true`, every file hit carries `version` — build `files_scope="{id}:{version}"` straight from the row; at `details=false` re-run with `details=true` (or GET …/details/) to get it.
