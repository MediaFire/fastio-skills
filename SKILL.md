...
**metadata-view-export** -- Asynchronously materialize the caller's saved view for a template into a TSV file written into workspace storage. Requires `workspace_id` and `template_id`. Optional `parent_node_id` is the destination folder OpaqueId; omit (or pass empty) to write to workspace root. The endpoint enqueues the work and returns immediately — the actual TSV is written by a background worker into the destination folder. Two response shapes: (a) **queued** — `{result:true, job_id, status:"queued", filename, message}` where `filename` is derived from the saved view's `name` (sanitized for filesystem-unsafe chars, `.tsv` extension; falls back to `metadata_view_{partialViewId}.tsv` if the sanitized name is empty); (b) **duplicate** — `{result:true, status:"duplicate", filename, message}` with NO `job_id` when an export for the same `(workspace, user, template, destination_folder)` is already queued or in flight. Same view exported to a different folder is not a duplicate. **Prerequisites**: caller must have a saved view (`metadata-view-save` first; otherwise 404 *View not found*); workspace org must have the metadata feature enabled (403 if not — billing-feature gate, Free/Pro/Business all qualify). **To detect completion**, poll the destination folder via `storage` action `list` (or via `storage` action `details` on the parent) and look for a node whose `name` matches the returned `filename`; the in-folder approach is robust against the duplicate branch (no `job_id` to track). Most exports complete within seconds. **Concurrent exports** within a single workspace are serialized by a workspace-wide Redis mutex — two callers exporting different views in the same workspace will be processed one at a time. **Snapshot semantics**: the TSV reflects the metadata state at the moment the worker runs, not at request time. **No customizing the filename or columns** — both come from the saved view; update the view first via `metadata-view-save` to change them. Per-user views: a workspace admin cannot export "the team's view"; the export reflects whatever the calling user has saved. The endpoint is rate-limited per-user and per-workspace (X-Rate-Limit-* headers).

**metadata-search** -- Lexical keyword search over template + custom metadata field values for a workspace. Returns workspace files whose metadata field values contain the query keyword(s), scored by relevance. Requires `workspace_id` and `q` (search keyword(s); whitespace-trimmed; empty rejected; max 1024 chars). Optional `template_id` restricts results to nodes with at least one value contributed by that template (custom fields are excluded when this filter is set). Optional `limit` (1-100, default 100) and `offset` (default 0) for pagination — `offset+limit` may not exceed 10000. **Search semantics**: multi-token queries (e.g. `q=foo bar`) require ALL tokens (analyzer-tokenized AND); case-insensitive; **substring matching applies for queries ≤64 chars**; for longer queries only whole-word/token matching applies (a 200-char prose query is matched word-by-word, not as a substring). Returns `{results, pagination}`: each result entry is `{node_id, score (BM25 — relative sort signal only, not comparable across queries), template_ids (which templates contributed values; empty array means custom-field-only match), node (full hydrated node payload, same shape as `storage` action `details` — trashed nodes filtered out at hydration)}`. Pagination shape: `{total, limit, offset, has_more}` where `total` is the ES hit count (post-instance filter, pre-trash filter — use it for "X results" UI labels) and `has_more = (offset+limit) < total`. **Indexing latency**: metadata writes propagate to the search index asynchronously via the event queue — expect a 1-2s window after a metadata edit (template extraction, automatch, manual edit) before the new value is queryable. **Don't search-immediately-after-writing as a correctness check** — it'll race; for "show what was just saved" flows, read straight from the metadata template endpoints, not from search. **Tenancy**: the server always injects `instance_id = workspace_id` into the underlying ES filter; cross-workspace coercion is not possible. Passing another workspace's `template_id` just yields zero hits (no leak). **What it is not**: not a structured filter (use `metadata-list-files` with `metadata_filters` for `where field = value`), not semantic/vector search (lexical only — `"house"` won't match `"residence"`), not for searching file contents (use `storage` action `search` for that). For result snippets/highlights, the API does not return them; use the matched `node_id` to fetch metadata via `metadata-get` and substring-match client-side.
